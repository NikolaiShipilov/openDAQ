...
DECLARE_OPENDAQ_INTERFACE(ICredentialPayload, IBaseObject)
{
     ...
     * `IDict<IString, IString>` for a `KeyValuePairs`-format payload. Callers are expected to know the
     * format (e.g. from the IAuthenticationConfig and ICredentialPayloadDescriptor used to request the
     * credential) and cast accordingly.
