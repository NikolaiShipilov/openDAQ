...
3. **Resolve the credentials.** `Module::obtainCredentials` reads `getCredentialProviderId()` and `getSuppliedSecret()` off the authentication config, and branches on whether the latter returned one:
